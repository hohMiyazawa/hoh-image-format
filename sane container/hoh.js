		...
		let sharpener = function(a,b,resolver,errorFunction,symbol){
			...
			if(options.forceGradients){
				...
				while(new_error < error){
					a = new_a;
					error = new_error;
					new_a = Math.min(table_ceiling,Math.max(a + diff,0));
					new_error = errorFunction(resolver(new_a,b))
				}
				let new_b = Math.min(table_ceiling,Math.max(b - diff,0));
				new_error = errorFunction(resolver(a,new_b));
				while(new_error < error){
					b = new_b;
					error = new_error;
					new_b = Math.min(table_ceiling,Math.max(b - diff,0));
					new_error = errorFunction(resolver(a,new_b))
				}
			}
			return {
...
//end debug
		...
		console.log("freaks",DEBUG_integer_f);
		if(options.quantizer === 0){
			for(let i=0;i<width;i++){
				for(let j=0;j<height;j++){
